            ...
            def correctMatches(srcList):
                ...
                for alg in algorithms:
                    if alg in schema:
                        ...
                        for src in srcList:
                            src[key] /= ratio
            if matchesType is None or matchesType in (self.MATCHES_FIRST, self.MATCHES_BOTH):
                correctMatches([match.first for match in matches])
            if matchesType is None or matchesType in (self.MATCHES_SECOND, self.MATCHES_BOTH):
