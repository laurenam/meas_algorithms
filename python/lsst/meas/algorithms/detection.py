...
class SourceDetectionConfig(pexConfig.Config):
    ...
    shrinkMask = pexConfig.ListField(dtype=str, default=[], doc="List of mask bits for shrinking footprints")
...
class SourceDetectionTask(pipeBase.Task):
    ...
    def detectFootprints(self, exposure, doSmooth=True, sigma=None, clearMask=True):
        ...
        for polarity, maskName in (("positive", "DETECTED"), ("negative", "DETECTED_NEGATIVE")):
            ...
            if len(self.config.shrinkMask) > 0:
                originalSize = len(fpSet.getFootprints())
                splits = afwDet.FootprintList()
                for fp in fpSet.getFootprints():
                    fp.shrink(maskedImage.getMask(),
                              maskedImage.getMask().getPlaneBitMask(self.config.shrinkMask))
